...
router.get("/", async (req, res) => {
  try {
    ...
    const monthlyRevenue =
      monthlyRevenueResult.length > 0 ? monthlyRevenueResult[0].revenue : 0;

    // 5. Today's revenue
    const startOfDay = new Date(now.setHours(0, 0, 0, 0));
    const todaysRevenueResult = await Sale.aggregate([
      { $match: { createdAt: { $gte: startOfDay } } },
      { $group: { _id: null, revenue: { $sum: "$total" } } },
    ]);
    const todaysRevenue =
      todaysRevenueResult.length > 0 ? todaysRevenueResult[0].revenue : 0;
